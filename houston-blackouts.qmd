...
::: {layout-ncol="2"}
![Night light intensity in Houston, TX prior to the winter storms as seen via satellite.](outputs\map_0207.png)

![Night light intensity in Houston, TX prior to the winter storms as seen via satellite.](outputs\map_0216.png)
:::
...
```{r}
# set map bbox
bbox <- st_bbox(c(xmin = 1815753,
                 ymin = 7189634,
                 xmax = 2044850,
                 ymax = 7420456),
                crs = "EPSG: 3083")

map_blackout_homes <- tm_shape(counties, bbox = bbox2)+
...
```{r}
map_blackouts_tracts <- tm_shape(counties, bbox = bbox)+
  tm_polygons(col = "#1e0057",
              fill = "#1e0057",
              fill_alpha = 0.7)+
  tm_shape(houston_counties) +
...
ggsave(here("outputs", "plot_blackout_tracts.png"), plot_blackout_tracts,
       height = 4, width = 7, dpi = 400, bg = "transparent")
```
